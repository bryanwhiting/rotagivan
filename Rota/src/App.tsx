...
function HudPage({ state, profile, layer, layerIndex, selectLayer, update, updateLayer, navigate, open }: { state: RotaState; profile: Profile; layer: HudLayer; layerIndex: number; selectLayer: (id: string) => void; update: (recipe: (draft: RotaState) => void) => void; updateLayer: (recipe: (layer: HudLayer) => void) => void; navigate: (direction: HudDirection) => void; open: () => void }) {
  ...
  const addLayer = (position: HudDirection) => {
    ...
    update(draft => {
      ...
      const tiles = Array.from({ length: 6 }, () => ({ id: crypto.randomUUID(), label: "Empty", icon: "+", action: "Unassigned" }));
  ...
  const moveLayer = (id: string, position: HudDirection) => update(draft => {
    const current = draft.profiles.find(item => item.id === profile.id)!;
    const moving = current.layers.find(item => item.id === id);
    if (!moving || moving.id === "main" || moving.position === position) return;
    const previous = moving.position;
    const occupant = current.layers.find(item => item.id !== "main" && item.id !== id && item.position === position);
...
function PointerPage({ profile, update }: { profile: Profile; update: (recipe: (profile: Profile) => void) => void }) {
  ...
  return <div className="page two-up"><Card title="Pointer response" icon={MousePointer2}><MotionGraph speed={pointer.cursorSpeed} acceleration={pointer.cursorAcceleration} color="#39acec"/><Range label="Speed" value={pointer.cursorSpeed} onChange={value => update(draft => { draft.pointer.cursorSpeed = value; })}/><Range label="Acceleration" value={pointer.cursorAcceleration} onChange={value => update(draft => { draft.pointer.cursorAcceleration = value; })}/><Range label="Smoothing" value={pointer.smoothing} onChange={value => update(draft => { draft.pointer.smoothing = value; })}/></Card><Card title="Scroll response" icon={SlidersHorizontal}><MotionGraph speed={pointer.scrollSpeed} acceleration={pointer.scrollAcceleration} color="#7ccfff"/><Range label="Speed" value={pointer.scrollSpeed} onChange={value => update(draft => { draft.pointer.scrollSpeed = value; })}/><Range label="Acceleration" value={pointer.scrollAcceleration} onChange={value => update(draft => { draft.pointer.scrollAcceleration = value; })}/><Range label="Coast" value={pointer.coast} onChange={value => update(draft => { draft.pointer.coast = value; })}/><Toggle checked={pointer.kinetic} onChange={value => update(draft => { draft.pointer.kinetic = value; })} label="After-scroll coasting"/><div className="check-pair"><Toggle checked={pointer.invertX} onChange={value => update(draft => { draft.pointer.invertX = value; })} label="Invert horizontal"/><Toggle checked={pointer.invertY} onChange={value => update(draft => { draft.pointer.invertY = value; })} label="Invert vertical"/></div></Card><Card title="Dragging" icon={Command} className="span-two"><Toggle checked={pointer.tapDrag} onChange={value => update(draft => { draft.pointer.tapDrag = value; })} label="Tap, then hold to drag" detail="Lift and reposition without dropping"/><Toggle checked={pointer.regrip} onChange={value => update(draft => { draft.pointer.regrip = value; })} label="Allow re-grip" detail="Keep the synthetic drag active during repositioning"/></Card></div>;
}
