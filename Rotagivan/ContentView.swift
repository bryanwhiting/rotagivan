        ...
        VStack(alignment: .leading, spacing: 22) {
            ...
            Section("Apple trackpad · actions only") {
                ...
                Text("Uses the active layer’s tap and swipe bindings for shortcuts, App Explorer, and Window Manager. Enable tap actions in that layer to use tap bindings.")
                    .font(.callout)
                Text("macOS keeps its normal cursor and scrolling behavior outside the HUD. While an Apple-controlled App Explorer or Window Manager is open, pointer movement pauses so you can swipe to select. Closing the HUD or editing it restores movement. This briefly holds the shared pointer still for other mice too. Clicks, scrolling, and keyboard input remain available.")
                    .font(.caption).foregroundStyle(.secondary)
                Text("Motion settings and synthetic click/drag bindings apply only to Navigator. Other native macOS gestures are not blocked, so overlapping gestures may trigger both actions.")
                    .font(.caption).foregroundStyle(.secondary)
