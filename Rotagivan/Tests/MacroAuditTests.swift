...
@main struct MacroAuditTests {
    static func main() {
        ...
        var first = ExplorerHoldLayer(name: "First", holdShortcut: nil, launchShortcut: key, appBundleID: "app.first")
        ...
        precondition(!audit().findings.contains { $0.kind == .conflict }, "Disjoint app launchers cannot conflict")
        first.appBundleID = nil; settings.appExplorer?.holdLayers = [first, second]
        precondition(audit().findings.contains { $0.kind == .conflict }, "Global and scoped launchers overlap")
        ...
        print("Macro audit passed: per-step interception, missing targets, and app-scoped launcher overlap")
    }
}
