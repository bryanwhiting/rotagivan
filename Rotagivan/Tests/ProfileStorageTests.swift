...
struct ProfileStorageTests {
    @MainActor static func main() throws {
        ...
        precondition(store.removeProfile(1), "The current default layer should be removable when another layer exists")
        precondition(store.defaultProfileID == 2 && store.activeProfileID == 2)
        precondition(store.settings.removedLayerIDs == [1])
        precondition(store.profiles.map(\.id) == [2])
        precondition(!store.removeProfile(2), "The final layer must remain protected")
        let persistedRemoval = try decoder.decode(StoredSettings.self, from: encoder.encode(store.settings))
        precondition(persistedRemoval.removedLayerIDs == [1] && persistedRemoval.availableLayerIDs == [2])
        print("Layer deletion passed: custom and built-in cleanup, default promotion, persistence and final-layer protection.")
