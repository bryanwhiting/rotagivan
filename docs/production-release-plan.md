...
### Voice feedback and explicit Keychain unlock — 1.1.185 (187)

- Background login/vault restoration now uses non-interactive Keychain reads. If access requires permission, General → Account & Sync offers an explicit retry/unlock instead of opening a password dialog during startup or voice activation. Successful vault restoration is cached in memory for the active account; repeated voice commands do not reread Keychain. Account changes and shutdown clear this cache. No Keychain items, ACLs, signing trust, or Accessibility permissions are reset.
- Existing legacy login-keychain items are retained. The serialized credential worker suppresses UI for each background read and immediately restores the process-local interaction policy. Failure to suppress UI fails closed. This follows Apple's [authentication UI controls](https://developer.apple.com/documentation/Security/kSecUseAuthenticationUI) and [legacy Keychain interaction controls](https://developer.apple.com/documentation/security/keychains?language=objc); it is not a migration to the data-protection Keychain. Explicit unlocking may still require the user's login password.
- Voice now distinguishes preparation/listening/transcription/ranking from an actual no-match result. Returned preview candidates show readable names and scores; alternatives remain visible when Jev chooses none. Previews and rejected candidates cannot execute. Final valid choices retain swipe-and-lift activation. Partial-request failures now show a status instead of silently disappearing.
- This is feedback for actual snapshot responses, not token streaming, internal model reasoning, or a new Jev WebSocket transport. Live-provider latency and physical gesture testing remain separate verification gates.
- Focused verification passed: `VaultCryptoTests`, `CredentialVaultTests`, `VaultRestoreUISmoke`, `VoiceTests`, `VoiceHUDUISmoke`, `ManualSyncTests`, `CredentialLifecycleTests`, and `SyncStorageTests`. Injected tests cover silent-read policy restoration/fail-closed behavior, explicit-only unlocking, repeated cached voice reads, account-change rejection, and visible-but-disabled preview/no-match candidates. Final HUD captures were reviewed at `/private/tmp/rotagivan-hud-empty-circles/Rotagivan/build/voice-preview-verification`. No real credentials, item ACLs, or live voice providers were accessed by these tests.
- Built, installed using `./launch.sh --keep-accessibility`, and reopened from `/Applications/Rotagivan.app`. Version/build, running executable, strict signature, and persistent certificate pin were independently verified. Built/installed executable SHA-256 matches `7b20960d057c68d723e12aa79883a2e074ff2f9f765e490df00aba836143da78`; source fingerprint stayed unchanged through compilation. Unfinished note-entry work was excluded. Existing-item permission behavior on another Mac and live-provider speech remain unverified.
